...
public class PlayerController : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			if(l_AnimatorStateInfo.fullPathHash==l_DeadIdAnimation && l_AnimatorStateInfo.normalizedTime>=1.0f)
				RestartGame();
			return;
		}
		...
		if(m_CanShootGrenade && l_AnimatorStateInfo.fullPathHash != l_ThrowGrenadeIdAnimation && m_Grenades>0 && l_ThrowGenadePressed)
		{
			m_Animator.SetBool("ThrowGrenade", l_ThrowGenadePressed);
			ThrowGrenade();
		}

		if(m_CurrentGrenade!=null && l_AnimatorStateInfo.fullPathHash == l_ThrowGrenadeIdAnimation)
		{
